...
const MajorChange = ({ milestones }) => {
  const isMobile = useBreakpointValue({ base: true, md: false })
  const isDesktop = useBreakpointValue({ base: false, md: true })

  // insert id into milestones object on each element incrementally
  const milestonesWithId = milestones
    .map((milestone, index) => {
      return { ...milestone, id: index }
    })
    .reverse()
  // const gridColumn = useBreakpointValue({ base: 1, md: 2 })
