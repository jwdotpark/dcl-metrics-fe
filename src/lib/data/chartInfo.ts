...
export const plotMissingDataArr = (data) => {
  // Create an array of dates in the range of the data
  ...
  // Create an object with the data indexed by date
  const dataByDate = {}
  for (const d of data) {
    dataByDate[d.date] = d
  }

  // Plot the data, filling in missing dates with 0 time_spent
  const plotData = []
