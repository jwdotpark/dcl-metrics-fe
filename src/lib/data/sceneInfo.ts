export const description = {
  visitors: "Unique visitors yesterday in this scene",
  share_of_global_visitors:
    "Percent of total DCL visitors which visited this scene yesterday",
  avg_time_spent: "Average time users spent yesterday in this scene",
  avg_time_spent_afk: "Average idle time users spent yesterday in this scene",
  total_logins:
    "The total number of times users logged in directly to this scene",
  unique_logins: "Count of unique users that logged in directly to this scene",
  total_logouts:
    "The total number of times users logged out directly from this scene",
  unique_logouts:
    "Count of unique users that logged out directly from this scene",
  complete_sessions:
    "Number of times that users logged in and logged out of this scene without visiting any other scenes",
  avg_complete_session_duration:
    "Average time spent by users with complete sessions",
}

export const name = {
  visitors: "Visitors",
  share_of_global_visitors: "Share of Global Visitors",
  avg_time_spent: "Avg Time Spent",
  avg_time_spent_afk: "Avg Time Spent AFK",
  total_logins: "Total Logins",
  unique_logins: "Unique Logins",
  total_logouts: "Total Logouts",
  unique_logouts: "Unique Logouts",
  complete_sessions: "Complete Sessions",
  avg_complete_session_duration: "Avg Complete Session Duration",
}
